...
**By:** Thandokuhle Brian Msane and Thandolwethu Agnes Nhlabatsi \
...
Producer-Consumer/
│
├── buffer/                # Shared directory for XML files which acts as a buffer
├── src/
  ...
  ├── main.py              # runs the producer, consumer and server processes using threads
  ├── student.xsd          # XML Schema for validation
  ├── student.log          # logging file for system log events
  ├── pyproject.toml       # requirements file for both development dependencies like `pre-commit` and `ruff` for code linting
  ├── uv.lock              # `uv` dependency management file
